            ...
            <div className="rounded-lg shadow-md border border-gray-200 bg-white p-4 text-sm space-y-3">
                <div className="flex justify-between items-start gap-2 ">
                    <p className="font-semibold text-gray-700 break-all w-[150px] md:w-[450px] ">
                        ID:<span className="text-black text-xs md:text-sm px-1 py-0.5 rounded">{order.order_id}</span>
                    </p>
                ...
                <div className="flex justify-between items-center">
                    <div className="flex items-center gap-2 text-gray-700">
                        ...
                        <span className="font-semibold">
                            <p className="text-sm">
                                Total: <span className="text-gray-500 line-through mr-2">₹{order.total_amount}</span>
                                <span className="text-black font-semibold">₹{order.discounted_amount}</span>
                            </p>
                        </span>
                    </div>
            ...
            {showOtpPopup && (
                <div className="inset-0 z-50 backdrop-blur-sm bg-black/30 fixed flex justify-center items-center">
                    <div className="bg-white p-6 rounded-lg shadow-lg w-[90%] max-w-sm">
                        <h3 className="text-lg font-bold text-gray-700 mb-2">Enter OTP</h3>
                        <input
                            type="text"
                            value={otp}
                        ...
                        <div className="flex justify-end gap-2">
                            ...
                            <button
                                onClick={async () => {
                                    ...
                                    const { success } = await updateOrderStatus(order?.order_id, 'on the way', otp);
                                    if (success) {
