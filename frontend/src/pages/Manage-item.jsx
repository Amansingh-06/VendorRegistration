  ...
  const toggleAvailability = async (id, currentValue) => {
    ...
    if (error) {
      ...
      if (Object.keys(changes).length > 0) {
        ...
        }" availability toggled for vendor ID ${selectedVendorId}. Changes: ${generateChangeDescription(
          changes
        )}`;
    ...
    <section>
      <div className="max-w-2xl mx-auto rounded-2xl shadow-lg">
                ...
                <div className="flex flex-1 sm:flex-row items-start   gap-2">
                  ...
                  <div className="flex-1 flex flex-col    ">
                    <div className="flex items-start   gap-1">
                      ...
                      <div
                        className={`p-1 rounded-lg mt-1 ml-0.5   ${
                          item?.veg === true ? "bg-green-100" : "bg-red-100"
                        }`}
                      >
                        <span
        ...
        ${item?.veg === true ? "bg-green-600" : "bg-red-600"}`}
                    ...
                    <p className="text-gray-600  p-1 text-base flex items-center gap-1 ">
                      <GiFruitBowl />{" "}
                      <span className="text-gray-600 text-[13px]">
                        Quantity: {truncateLetters(item?.item_quantity, 8)}
                      </span>
                    </p>
                    <p className="text-gray-600  p-1 text-sm flex items-center gap-1">
                      <FaRegClock />{" "}
                      <span className="text-gray-600 text-[13px]">
                        Prep Time: {truncateLetters(item?.prep_time, 8)}min
                      </span>
                    </p>
                    <p className="text-gray-600  p-1 text-sm flex items-center gap-1">
                      {" "}
                      ...
                      <span className="text-gray-600 text-[13px]">
                        {" "}
                        Price: ₹{truncateLetters(item?.item_price, 8)}
                      </span>
                    </p>
                  </div>
                ...
                <div className=" flex justify-between py-3 gap-5 ">
                  <div className=" flex w-32   items-center  md:flex-0 gap-1 bg-gray-100 rounded md:px-3    shadow-sm">
                    <Switch
                      checked={item.available}
                      onChange={() =>
                        toggleAvailability(item.item_id, item.available)
                      }
                      size="small"
                      color="success"
                    />
                  ...
                  <div className=" sm:mt-auto sm:ml-auto flex  flex-1 justify-end gap-5 md:justify-evenly  items-center ">
                    <button
                      onClick={() =>
                        navigate("/add-items", { state: { itemData: item } })
                      }
                    ...
                    >
                      <FiEdit className="text-blue-600 group-hover/btn:scale-110 transition-transform duration-200 w-[15px] h-[15px] lg:w-[18px] lg:h-[18px]" />
                    </button>

                    <button
                      onClick={() =>
                        confirmDelete(item.item_id, item.item_name)
                      }
                      className="transition-all duration-300 transform hover: scale-98 p-2 bg-white bg-opacity-80 hover:bg-opacity-100 rounded-lg shadow-all hover:shadow-md group/btn cursor-pointer"
                    >
                      <FiTrash className="text-red-600 group-hover/btn:scale-110 transition-transform duration-200 w-[15px] h-[15px] lg:w-[18px] lg:h-[18px]" />
                    </button>
                  </div>
                </div>
              </div>
        ...
        <div className="fixed bottom-18 right-4 sm:right-6 md:right-15 lg:right-49 lg:bottom-18 xl:right-110 xl:bottom-18 z-50">
          <div className="flex flex-col items-center gap-2">
            ...
            <span className="text-[11px] sm:text-xs md:text-sm text-orange font-medium">
              Add Item
            </span>
          </div>
        </div>
        ...
        <div className="inset-0 z-50 backdrop-blur-sm bg-black/30 fixed flex justify-center p-5 items-center">
          <div className="bg-white rounded-xl shadow-xl p-6 max-w-sm w-full text-center">
            ...
            <p className="text-gray-600 mb-6">
              ...
              <strong>{truncateLetters(deleteItemName, 17)}</strong>
            </p>
